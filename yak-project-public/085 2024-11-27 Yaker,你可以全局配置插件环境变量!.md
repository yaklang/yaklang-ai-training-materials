...
日期: 2024-11-27 | 原文: <https://mp.weixin.qq.com/s/NcuLj5aMmNmhtcztIDUUOw>

周四周四，Vme50(bushi

大家好，这里是疯狂超级牛（功能上新版）

![image](static/d49468ebc97d50f2.png)
...
![image](static/7ce1e95baccc5393.png)

![image](static/fa717fef505f058f.png)
...
![image](static/42b3f4a5eeab6490.png)

可以看到上述demo中为cli参数 env设置 setPluginEnv之后，参数预览处就没有对应的输入框了。

![image](static/59523e81cf8324bb.png)

插件环境变量可以在两处配置 插件商店页面的配置页 和 单个插件的配置页

![image](static/be1eea22b607bf89.png)

这两处的配置有一些细节的不同。

![image](static/079ac5765fb85862.png)

![image](static/ddd93f23e1b6f853.png)

这里的配置页面拥有完整的对环境变量的增删改查能力。可以新建环境变量，即使没有插件使用。

![image](static/812ace38b2c1a1d4.png)

需要说明的是变量是**可以支持空值**的，**这与没有环境变量并不等价。**

![image](static/adabd04b2fa4e6f3.png)

这部分的配置是为了方便使用做出的针对单个插件的简单版配置

![image](static/e488ef8d6298c038.png)

它只对本插件代码中使用的环境变量进行了展示，只提供对应的修改功能。

这里是对代码进行了解析处理，如果有代码中需要使用的环境变量没有被配置的话，会在此处提示用户。

![image](static/2fd05a68e255a8bd.png)

点击配置即可在此处快捷配置环境变量

> 需要注意的时候环境变量的影响范围是全局，这里修改之后，所有的使用对应环境变量的地方都会受到影响。

![image](static/a489c79a78220faa.png)
...
![image](static/7f5a13493ae75acc.png)

点击配置设置好环境变量之后执行插件，可以看到成功地进行请求头插入

![image](static/5e2e229288227741.png)
