...
日期: 2022-05-10 | 原文: <https://mp.weixin.qq.com/s/QF85PJETvSpoOoXK1Bl02w>
...
![image](static/ede5d66dc8dbd95c.png)
...
![image](static/89d012507d6840a5.png)

再设置远程调试，如图

![image](static/a4051cf5b2707750.png)
...
![image](static/43a3a43ca2cced3b.png)
...
![image](static/6b9f1bf2b82ca7ab.png)
...
![image](static/34b632a7994e9a6b.png)
...
![image](static/827f24668778ed79.png)

所以只需要将m_comparator改为恶意ChainedExtractor，m_oAnchorTop改为Runtime.class，就可以反射调用Runtime的getRuntime->exec，至于toString方法，可以通过BadAttributeValueExpException触发（CC5链学过），这就是CVE-2020-2555的利用链，更新补丁将LimitFilter的toString方法，去掉了extract的调用

但翻看Coherence组件，发现有ExtractorComparator类，可以作为comparator。回忆下CC链，PriorityQueue就用到了comparator：readObject()->heapify()->siftDown()->siftDownUsingComparator()->this.comparator.compare()，所以可以PriorityQueue+ExtractorComparator构造，这就是CVE-2020-2883的利用链

![image](static/d85171aa4e129961.png)

### **回显利用**

因为ReflectionExtractor可以调用任意对象的任意方法，所以可以构造TemplatesImpl实现代码执行，https://github.com/Y4er/CVE-2020-2555项目安装rmi的payload利用链是AnnotationInvocationHandler ->LazyMap->Transformer然后defineClass，再invoke
...
![image](static/d459fad58db90e83.png)

authentication可以为null(上文有截图，令前缀的length小于at.getCapacity()即可)

context组成：

ClassTableEntry(PackageInfo)、ClassTableEntry(VersionInfo)、ClassTableEntry(PeerInfo),authentication,jvmid,jvmid（PackageInfo 是VersionInfo 的父类，VersionInfo 是PeerInfo的父类）

lookup组成：

ImmutableServiceContext(methodDescript("getNameInNamespace()"))，authentication
...
![image](static/10b814d9f59d9852.png)
