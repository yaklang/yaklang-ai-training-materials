...
日期: 2023-03-30 | 原文: <https://mp.weixin.qq.com/s/mBe-e_kAXi5PxPqkpVJzGw>
...
![image](static/1697cb8b708829c0.png)
...
![image](static/a6b6e96050aeebd2.png)
...
![image](static/a3c47bf41bd48766.png)
...
![image](static/7cb0ce0fc3a18ca5.png)

返回到ldapCtx

![image](static/244c674cf307ef80.png)

![image](static/38db4c0c59451751.png)

在此处实例化了远程引用var3

跟进DirectoryManager中的getObjectInstance方法

![image](static/07bd70b885d3a5f0.png)

核心点

![image](static/0025ad4168aba176.png)

factory = getObjectFactoryFromReference(ref, f);

![image](static/d566d1c854ad8e4f.png)

先尝试本地加载，否则用codebase加载。最终因为加载我们的远程恶意类，再静态方法区嵌入恶意代码，实例化过程中被执行

![image](static/46895677d807d44b.png)

![image](static/2b002aa1cd08bde3.png)

##### **java>8u191 修复原理，对使用codebase进行URLClassloader加载前进行trustURLCodebase判断**

在使用远程codebase进行loadClass时不会像上面这样直接去Class.forName()而是

![image](static/7069ea28cde20b29.png)

可以看到对trustURLCodebase进行了判断，只有为true才会进行实例化。

![image](static/53c257cc9acbff15.png)
...
![image](static/2421524bb688c92e.png)
...
![image](static/331cbcc14f1330d8.png)

![image](static/43941b976ee2c10b.png)
...
![image](static/002d363749f4024d.png)
...
![image](static/2ac4762dd5f1f458.png)
...
![image](static/cca4674a6f6ca0d0.png)

![image](static/349ad7b4ee82d214.png)

var3，这里又和情况1，加载远程codebase中的reference有点类似了，但是此处为ResourceRef，继续跟进

![image](static/02b3d52783ed7d02.png)

熟悉的getObjectInstance，从Reference中拿instance

![image](static/a589433cb071d4d8.png)
...
![image](static/22db182471b4c454.png)

从ref中拿我们指定的factory

![image](static/808ffa7f36400279.png)

![image](static/68142ac428691720.png)

这里就是初始化本地的factory

![image](static/41025990c511576c.png)

**核心点**，调用了factory的getObjectInstance()我们跟进看看

![image](static/8feaaaf8cfcc288a.png)
...
![image](static/f8447603a8c28bf1.png)

可以看到这里主动从ref中拿属性

![image](static/2fad80c628c767d4.png)

![image](static/4a695881cbfe44a7.png)

![image](static/c681db0120bc5a18.png)
...
![image](static/4d6e7ad05cda60fa.png)
