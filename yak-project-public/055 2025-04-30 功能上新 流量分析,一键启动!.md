...
日期: 2025-04-30 | 原文: <https://mp.weixin.qq.com/s/gc6qEpEsZBeksAZYBEgRTQ>

![image](static/651286fda5f61257.webp)

![image](static/8f13c6b879785bdc.jpg)

![image](static/79e1d0e4b6f4277f.png)

为什么需要流量分析功能呢？

在以往，我们一般通过 `History `页面来处理已经存进数据库的流量，通过关键词、网站树等方法筛选出我们想要的流量。

![image](static/f15e807ce1a19641.png)
...
![image](static/632deef89d29ea65.png)

我们先大体介绍一下 Yakit 流量分析的功能分布以及运行流程。

![image](static/0113fe94fbe21df3.png)

流量分析功能页面的入口在**History**右边的小图标上面：

![image](static/f7f59bfab67f3c8f.png)

![image](static/8a94db365db4294c.png)
...
3. 配置页面。这个页面是一些流量分析功能一些配置信息，包括数据类型、并发以及单条记录的去重等。 ![image](static/74e39cab3a6721e7.png)

流量分析是个简单的输入输出模型，运行流程图如下：

![image](static/41d3131cb38e5a70.png)

接下来，文章会对 Yakit 的数据输入、流量分析、结果处理模块做个详细的介绍。

![image](static/94e838015c088291.png)

![image](static/c579df5dac0accfb.png)

选择数据的输入可以通过配置页面进行配置：

![image](static/8d8b440ca69419bf.png)
...
![image](static/8c2a2d926f0441d4.png)

![image](static/f991f4663aa4c9cf.png)
...
![image](static/c5e356cb5ebff441.png)
...
![image](static/ada68ffe0806b318.png)

![image](static/c117c918d356bc8c.png)

在流量分析完以后，可以查看匹配到的内容。如上图，可以看到两个“导出”的按钮：上面的导出为将本次所有提取的规则数据进行导出，下面的导出则针对单条数据匹配到的规则。

![image](static/d44e1733563a79ad.png)

![image](static/1fcae0b7898a76cc.png)

在使用 WebFuzzer 的时候，如果产生数据包过多，可以跳转到流量分析里面进行流量分析。

![image](static/58f3694fbdbe640c.png)

此时流量分析器的筛选页面就会默认筛选出刚才 WebFuzzer 产生的流量，从而可以进一步分析。这个方式在 WebFuzzer 产生大量数据包的时候，对于想要查找有价值的数据十分有用。

![image](static/4db9518b664bbd39.png)

![image](static/8adcc09d1816c33c.png)
