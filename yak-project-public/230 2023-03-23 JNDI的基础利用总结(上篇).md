...
日期: 2023-03-23 | 原文: <https://mp.weixin.qq.com/s/XpDn15ExYfO0fyu7vQ3i0A>

![image](static/39de6f03c2e727ff.png)
...
![image](static/5c6693f57fcf8e72.png)
...
![image](static/f8da9eb5f308ec0e.png)
...
![image](static/8ce83ce7e41e6240.png)
...
![image](static/6164cba4a26c84db.png)

![image](static/8a6384044b743a99.png)
...
![image](static/014471147c423a85.png)

继续跟进 Context ctx = NamingManager.getURLContext(scheme, myProps);

![image](static/5264b72d9cd985db.png)

这一步便会决定到底用啥解析传入的lookup的url，这里根据预先定义的常量和传入的scheme，调用对应的Factory

![image](static/030f8ebb34078e1e.png)

确定加载的类名

![image](static/eefd708afbf1176e.png)

![image](static/89a889779f5bbd55.png)

![image](static/cf5a410d63094c00.png)

经过逐级返回回到最初点

![image](static/e1cafa49d00f7d5e.png)

最终我们执行lookup的context便是ldapURLContext

![image](static/d77b794572a2a64e.png)

#### **决定执行查询的Context后lookup的逻辑又是怎么样的**

#### 这里不允许我们的query中含有?判断后如果符合要求不含?则调用父类GenericURLContext中的lookup方法

![image](static/df50fd60e6c12cc1.png)

这里进一步对我们传入lookup进行查询的参数进行更细致的解析

![image](static/b7c0c1e673ef390a.png)
...
![image](static/40b606d270addb96.png)

调用ComponentContext中的p_lookup()方法

![image](static/3a38aa4cf524872e.png)

再调用LdapCtx中的c_lookup，这里便是处理具体的ldap协议栈的逻辑部分

![image](static/3fc02af6e4e26df4.png)

![image](static/5bd89f29420963c2.png)

这里对var1也就是SimpleCommand进行了查询，观察到ldap服务端检测到客户端请求

![image](static/8cc888af938dcb43.png)

查询后便会返回LdapResult

![image](static/9f960e9b4af83c61.png)

![image](static/c9dad09dc8c43e08.png)

返回结果比较有意思的就是entries中的LdapEntry了

一般正常完成查询他的状态码应该为0

![image](static/22b566f207f0cbcd.png)

这里对entry做一些处理拿到其中的属性attribute，存入var4

![image](static/bd9b4ac68b7daea6.png)

这里便会处理返回结果中的entry。一切的前提条件都是javaClassName属性不为空

![image](static/6cedec449b48c1ab.png)

其中值得关注的点便是JAVA_ATTRIBUTE常量数组

![image](static/44d7d666f65ffe60.png)

也就是所谓的javaClassName或者说CLASSNAME

![image](static/a847522269176446.png)

此处显然就是指SimpleCommand这个类

![image](static/4113dceebaaf0f73.png)

这里先拿codebase， 也就是恶意的远程codebase

![image](static/99cac5ab7385ad9c.png)

此处为核心点把这个方法拿出来分析

![image](static/0013cc168c314a0e.png)
