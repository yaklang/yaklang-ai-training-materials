...
![image](static/16522d209deca8a9.png)

代码如下，在过滤中，只过滤掉了`../`，没有过滤windows的`..\\`

![image](static/4c5c4b548be84959.png)

#### 漏洞复现：

![image](static/fdef0e7a54f6d866.png)

### 【内置规则】后台RCE（CVE-2024-2561 漏洞复现）：

![image](static/68d997bca911e0c5.png)

代码如下，在正则中，拿到$type，然后直接进行拼接，匹配的是data伪协议。

![image](static/8a9eff9a8af084dd.png)

#### 漏洞复现：

在这里进行构造：

![image](static/d82732d398a02646.png)

![image](static/836da79627742ed7.png)
...
![image](static/c0f1700b00e15044.png)

unzip是将输入的path进行一定的拼接，然后进行unzip。

![image](static/4192fc7ec17e7baa.png)

在这里找到一个unzip的反向，下载zip，下载完zip之后，会存到固定路径中，然后执行unzip即可。

![image](static/2bb80acd9423a194.png)

#### 漏洞复现：

zip文件夹路径如图：

![image](static/4b86dc34ce01320c.png)

![image](static/114a5a8bf4e02b64.png)
...
![image](static/36bd249da66a5c6e.png)

会解压失败，原因是因为没有conver文件夹，不过问题不大，因为已经执行了解压操作，并且已经将文件解压到对应目录中。

![image](static/ad1e12e0496a25f4.png)

![image](static/4c3de49294d52da3.png)
...
![image](static/cd83dccfa4c4a3e6.png)
