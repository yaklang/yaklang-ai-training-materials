...
日期: 2024-04-25 | 原文: <https://mp.weixin.qq.com/s/MuUdMud4fyuocIWnULVyXw>

![image](static/7a618c02eb75a887.gif)

**前言**

![image](static/9ee6369688f8439e.png)
...
![image](static/3eb24da202a1169d.jpg)
...
对于yaklang语言的用户来说，此次升级是几乎无感知的，在升级的同时确保了 80% 的API无变动，但由于底层实现不尽相同，因此可能会存在某些不常用的函数（如`js.ASTWalk`，`js.GetSTType`）以及结构体方法（如Value）会存在某些差异，对此库用户的影响控制在合适的范围内。
...
![image](static/33a69dfecb88618e.png)

响应中显示`解密前端内容成功`，这证明我们已经成功将请求体进行加密，可以用于爆破，后续只需要在for循环填入需要爆破的用户名密码即可。
