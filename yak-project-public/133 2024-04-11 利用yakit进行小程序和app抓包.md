...
日期: 2024-04-11 | 原文: <https://mp.weixin.qq.com/s/ey-YftiIHHweP4pGfa58oQ>

**Proxifier 介绍**

![image](static/011b0de10e073f95.png)
...
![image](static/358873369ca9cfd0.png)

点击劫持启动。

**2、Proxifier 添加 proxy server:**

![image](static/eb7ed691397b3ba7.png)

添加完成后，点击 OK 保存。

**3、手动添加规则**

![image](static/4d3e410a3c6e5894.svg)

**Windows Rules配置Windows 下 Proxifier 添加 Proxification Rules:**

Add 添加一条规则，填写如下:

![image](static/b9768b2ef694ac85.png)

注意 Applications 填写 `wechatappex.exe`, Action 选择上一步添加的 proxy server, 点击 OK 保存。

![image](static/ac794afde927e601.png)

![image](static/4d3e410a3c6e5894.svg)

**Mac OS Rules配置Mac 下 Proxifier 添加 Proxification Rules:**

Add 添加一条规则,通过`+`按钮点击:

![image](static/4933a3ee0d9a12a1.png)

通过 command+shift+G 打开 `/Applications/WeChat.app/Contents/MacOS` 路径

![image](static/d8c365939f96d786.png)
...
![image](static/0ea23eb95a44eafd.png)

Proxifier 会自动添加完毕。

![image](static/e8e3825ef2ebb48a.png)
...
![image](static/67b1d1c9fa6abd20.png)
...
![image](static/621a44c12e0ab2a2.png)
...
![image](static/709dbba626a1a797.png)

点击后可能会出现如下提示，让你保存当前的规则配置，如果你有其他的一些规则配置，可以点击取消后保存你之前的规则配置

![image](static/dcc49d2debd6d348.png)

这里我们没有需要保存的规则配置，直接点击是，选择下载的 Yakit-WeChatAppEx.ppx 文件

![image](static/56656c16d2f37a0e.png)

发现如下的加载成功即可

![image](static/911ca8c7bfc80ccf.png)

随后即可对小程序进行抓包

![image](static/36f0fb59b600fa95.png)

**常见错误排查**

![image](static/011b0de10e073f95.png)

在 Yakit v1.3.0 添加Windows 自定义安装路径之后，我们**需要对 Yakit 的证书进行重新安装**，并且证书安装完成后，需要**重启微信**。

证书最常见的错误就是，Yakit 证书安装错误，表现为打开小程序后，小程序显示网络错误，或者卡在加载界面，此时 Yakit MITM 也没有相应的流量。

网络错误、网络异常的情况

![image](static/62408f2a199de74b.png)

一直显示加载中的情况

![image](static/935579e94243648e.png)
...
![image](static/b99f695153477580.png)
...
![image](static/4b570f0d770ae8ff.png)

2.然后将证书复制到模拟器中（直接拖动到模拟器中即可），证书会出现在/storage/emulated/0/Pictures目录中

![image](static/ab523721cd6fd5b5.png)
...
![image](static/8e2bad96feaad24e.png)

将证书粘贴到  /etc/security/cacerts 目录下

![image](static/2b8c44a96d17b62a.png)

点开属性，并赋予权限

![image](static/39d6168853b87595.png)

5.访问app或网站即可抓到HTTPS的数据包

![image](static/70d79389671dba89.png)
...
![image](static/34438a30dcbf0fd6.png)

2.重命名“yakit证书.crt.pem”文件为38555061.0 （图中计算出的hash值）

![image](static/5e5b357db5cac41c.png)
...
![image](static/983dcd6ff318a52c.png)
...
![image](static/a7a603b5206025e3.png)

（7）在模拟器中设置-安全-信任的凭据中确认证书是否添加

![image](static/e26a5e45f6828d0b.png)

**最后**

![image](static/011b0de10e073f95.png)

本文较长，为了更好地让大家理解并学习，我们已在b站同步上传了视频，师傅们可以观看配套教程，使用Yakit上手实战。**点击文末阅读原文，观看视频。**
