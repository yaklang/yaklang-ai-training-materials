...
日期: 2022-05-27 | 原文: <https://mp.weixin.qq.com/s/0xQWHeTvimzMB7C6nQHEIw>
...
![image](static/13cf375b42c4690c.png)

fastjson漏洞分析

![image](static/8cce3cf5771527ed.png)
...
fastjson-1.2.24.jar!/com/alibaba/fastjson/parser/DefaultJSONParser.class#
...
![image](static/68e290371317e003.png)

这里的ref是value，即com.sun.rowset.JdbcRowSetImpl，然后通过类名加载对应类

![image](static/445aec5b7d17444d.png)

这里除了直接用类名，还有几种变形payload：
...
![image](static/c26d6b502510f64c.png)

如果是[开头，也是直接忽略掉，然后加载类，但返回的是

`com.sun.rowset.JdbcRowSetImpl`类型数组的class

类加载成功后，根据类型，获取deserializer(不妨叫它反序列化器吧)，开始“反序列化”

![image](static/d4a0b2953759a383.png)

反序列化过程在fastjson-1.2.24.jar!/com/alibaba/fastjson/parser/deserializer/JavaBeanDeserializer.class#deserialze方法中

大概流程就是，创建一个空对象，然后调用setXxx方法，为对象属性赋值，payload中有两个属性`dataSourceName`和`autoCommit`，`com.sun.rowset.JdbcRowSetImpl`的`setAutoCommit`方法会对`dataSourceName`进行lookup（相关代码在

com/sun/rowset/JdbcRowSetImpl.java），导致jndi或rmi注入

![image](static/2fa442dff1144724.png)

![image](static/2da5f43e2de157aa.png)
...
![image](static/4a93e2ff79efa08b.png)

反序列化器中调用了nextToken(12)，匹配了`{`，然后调用scanSymbol方法，但现指向字符是`,`，所以得到的key是null，进入下一次循环，得到key：dataSourceName
...
![image](static/bad963834859e36a.png)

### **第二个payload**

对比第一个payload，多了个

`{"@type":"java.lang.Class","val":"com.sun.rowset.JdbcRowSetImpl"}`，这个是对AutoType绕过的，1.2.25才有AutoType，

如图，原来直接loadClass改为了checkAutoType

![image](static/86c146dc57c7d9ad.png)

在checkAutoType方法中，未开启AutoType情况下会有下面两种加载方式，先看getClassFromMapping

![image](static/42eb8386d648a831.png)

如图，从mappings中取

![image](static/252d825faf7547ec.png)

第二种是从deserializers中取，如图deserializers是内置的一些类和对应的反序列化器

![image](static/8835b3b09b5d6adb.png)
...
![image](static/4d8a3b31c89daaf4.png)

最后一处在MiscCodec.deserialze方法中

![image](static/f7d07681d5bedef5.png)

MiscCodec继承了ObjectDeserializer，所以它应该反序列化器，所以去deserializers中看下这是谁的反序列化器

![image](static/71fdaf9a7b02c7e8.png)
...
`{"@type":"java.lang.Class","val":"com.sun.rowset.JdbcRowSetImpl"}`先加载一遍`com.sun.rowset.JdbcRowSetImpl`，放进mapping缓存，下次加载`com.sun.rowset.JdbcRowSetImpl`可以通过缓存加载，从而绕过AutoType的限制
...
![image](static/fed57c47ec171b48.png)
...
![image](static/32bc97be3aa91024.png)

![image](static/8f92922220c2fc5d.png)
...
![image](static/5f020a556274b80f.png)
...
**注：测试时发现有时在解析**json**前会判断类型是否匹配，匹配失败则在解析前抛出异常。所以需要将payload改造一下，如**

`**{"@type":"**``**java**``**.**``**net**``**.Inet4Address","val":"**``**xxx.dnslog.cn**``**"}**`**改为**
