...
日期: 2022-10-13 | 原文: <https://mp.weixin.qq.com/s/asKVnc24QMma_DrcbEvzwQ>

**反序列化**、**类加载**、**JNDI漏洞利用**是Java漏洞中特别常见的几种类型，但相对来说利用过程又是较为复杂的。**所以Yakit提供了两个特别好用的功能，无需Java环境，仅需一个Yakit。**

下面先介绍下使用方法再演示下实战利用。

![image](static/4c0d217ae7dca885.png)

反连服务器

![image](static/5221896f37cd7aef.png)
...
![image](static/2f19a2877d3435e9.png)

启动成功后如图，根据需求填写蓝色提示框内的地址.

![image](static/4c611d642f610467.png)
...
![image](static/3114991cbf6cc571.png)

在Yakit上可以看见收到请求，token为aaa，返回内容为<empty>（代表响应内容为空）

![image](static/4a46550d1d10e4d0.png)
...
![image](static/e89003ae2f0a78f0.png)

![image](static/c5a2add489110c4a.png)
...
![image](static/964ec8ee6f868cac.png)
...
![image](static/dfaeabec82226fa7.png)
...
![image](static/6a69ff528509665d.png)
...
2. 打开Yakit的端口监听器，监听一个端口，以 8086 为例 ![image](static/eb7cb87b9efae614.png)
3. 启动反连服务器并配置恶意类为 TCPReverseShell ，主机填 vps 的 ip ，端口填上面监听的 8086 ，点击应用

![image](static/b8d651de40e09c81.png)

1. 打开 webfuzzer，发送 Payload

![image](static/de83183144c89773.png)

![image](static/3908ab223a5a2206.png)

如图，反连列表中可以看见成功收到 LDAP 和 HTTP 请求，说明目标成功加载了恶意类（如果只收到 LDAP 请求，没有 HTTP 请求，可能是目标未开启

com.sun.jndi.ldap.object.trustURLCodebase ）

![image](static/92ffd6411ce8d9b8.png)

端口监听器收到了目标机器的连接

![image](static/544c6090dab81827.png)
...
![image](static/c179b9b4b2b02f40.png)

编写Yak脚本

![image](static/b375a708de5853f8.png)
...
![image](static/5ba06aab1136ad1f.png)

提交修改以后，可在“日志”查看修改记录，以及作者审核结果。

![image](static/ea6d3aa4ad1b228e.png)

为了帮助作者更好审核插件，贴心提供了插件源码的对比功能，一眼就可看出修改内容。

![image](static/5e53c5532d52e9f8.png)
