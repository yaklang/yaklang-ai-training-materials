...
日期: 2022-08-01 | 原文: <https://mp.weixin.qq.com/s/eCTyOTlfyJfp0wDK2mIlAQ>

![image](static/ca56f4ba8a03ed9f.gif)

**背景**

![image](static/59023e5fc9c7a5b5.png)
...
![image](static/caec9794f247376f.png)
...
![image](static/6cf72801085af0c3.png)
...
a.从客户端使用 nc -nv [your-server-ip] [port] 命令来判断端口是否开放；

b.Socat 类似的工具也都可以完成这个事情；

c.如果端口访问不通，需要排查服务器的安全策略组配置，或防火墙配置。

**2.端口通了，但是仍然无法连接？**

a.可以先卸掉所有认证机制，观察无认证是否可连通，再加上 TLS 与密码配置。

**3.版本问题：如果服务端和客户端版本相差特别大，将会出现协议不匹配的问题。升级引擎**
...
1. 生产环境下使用引擎服务器务必开启 TLS：可有效保护与服务器之间的通信不被窃；
2. 在开启了 TLS 之后，务必设置密码认证：以保护你的 Yak 引擎不被别人链接。
