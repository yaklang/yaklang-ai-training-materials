...
```yak title="使用 WavyCall 简化 fuzz.HTTPRequest"
raw = `GET / HTTP/1.1
Host: www.example.com
...
```yak title="WavyCall 创建对象，然后链式调用"
raw = `GET / HTTP/1.1
Host: www.example.com
...
```yak title="ExecFirst() (*_httpResult, error)"
raw = `GET / HTTP/1.1
Host: www.example.com
...
```yak title="模糊测试 Method，同时发送多个请求"
raw = `GET / HTTP/1.1
Host: www.example.com
...
```yak title="GetGetQueryParams 简化"
raw = `GET /target-path?abc=123 HTTP/1.1
Host: www.example.com
User-Agent: Ur-Awesome-UA/1.0001
...
```yak title="FuzzHTTPRequestParam 的成员方法描述"
...
```yak title="FuzzHTTPRequest 功能一览表"
