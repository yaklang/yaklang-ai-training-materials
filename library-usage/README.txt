...
- [x] **sandbox** - 安全沙箱、隔离执行、表达式求值
- [x] **facades** - 门面服务器、Java反序列化、JNDI注入
- [x] **openapi** - OpenAPI/Swagger解析、API文档、HTTP流生成
...
### 最新完成的库（本次更新）
- [x] **bin** - 二进制数据处理、字节流解析、文件格式分析（新增）
- [x] **sandbox** - 安全沙箱、隔离执行、表达式求值（新增）
- [x] **openapi** - OpenAPI/Swagger解析、API文档、HTTP流生成（新增）
- [x] **facades** - 门面服务器、Java反序列化、JNDI注入（新增）

### 之前完成的库
- [x] **spacengine** - 空间引擎、网络空间搜索、多搜索引擎集成
- [x] **mmdb** - MaxMind数据库、IP地理定位、访问控制
- [x] **ja3** - JA3指纹识别、TLS客户端识别、安全分析
- [x] **pcapx** - PCAP扩展、网络数据包捕获、流量分析
- [x] **cve** - CVE查询、漏洞数据库、风险评估
- [x] **cwe** - CWE查询、通用弱点枚举、弱点分类
- [x] **dictutil** - 字典工具、笛卡尔积运算、组合生成
- [x] **gzip** - Gzip压缩、数据压缩解压缩、格式识别
- [x] **tools** - 工具集、暴力破解工具、POC调用器
- [x] **suricata** - Suricata规则、IDS规则、网络入侵检测
- [x] **httpool** - HTTP连接池、批量请求、并发HTTP
- [x] **dyn** - 动态代码执行、Eval、Import
- [x] **hook** - 钩子函数、插件系统、动态加载
- [x] **yso** - YSO工具、Java反序列化、Gadget链
- [x] **t3** - WebLogic T3协议、JNDI、命令执行
- [x] **iiop** - WebLogic IIOP协议、远程调用
- [x] **rpa** - Web自动化、暴力破解、浏览器模拟
